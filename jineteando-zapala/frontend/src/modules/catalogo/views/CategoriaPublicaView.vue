...
<template>
  <div class="min-h-screen bg-[#FDFBF7] text-[#2C2A28]">
    ...
    <main class="max-w-6xl mx-auto px-5 py-12">
      <router-link to="/catalogo" class="text-amber-800 text-sm hover:underline mb-4 inline-block">&larr; Volver al catálogo</router-link>
        ...
        <template v-if="subcategorias.length > 0">
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <router-link
              ...
              class="group bg-white border border-gray-200 overflow-hidden hover:border-amber-800 transition-colors p-6"
            >
              <h2 class="text-lg font-medium group-hover:text-amber-900">{{ sub.nombre }}</h2>
            </router-link>
          </div>
        </template>
